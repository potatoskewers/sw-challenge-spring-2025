...
if __name__ == "__main__":
    ...
    # interface('1s', datetime.strptime('2024-09-17 12:34:01', "%Y-%m-%d %H:%M:%S"), datetime.strptime('2024-09-20 13:33:18', "%Y-%m-%d %H:%M:%S"), ctg_ticks)
    ...
    while True:
        ...
        if input_str == "quit": #quit interface
            break
        try:
            ...
            continue
        try: #attempt to extract values
            interval = values[0]
            start_time = values[1]
            end_time = values[2]
        except IndexError: #ask for input again if fail
        ...
        if interval is None or start_time is None or end_time is None: #handle bad input
            ...
            try: #attempt to convert to datetime and start interface
            ...
            except ValueError: #ask for input again if fail
